...
If the MCP server is unavailable because the connection has no credential, or if a tool reports an authentication or authorization error, stop the workflow and display the required access guidance below in the user's language. Ask the user in that language to configure the newly created token as the MCP connection's Bearer credential and reconnect. Never ask the user to paste the token into the conversation. Do not fall back to direct HTTP.
